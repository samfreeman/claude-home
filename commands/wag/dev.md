...
- Validates implementation against Architecture/PRD — including that code names **speak the ubiquitous language** (types, fields, columns, functions match Architecture's `Ubiquitous language` section, one name per referent; see `~/.claude/wag/references/ubiquitous-language.md`). Writes planning docs only (`.wag/docs/`, `.wag/adr/`, `.wag/snags/`) for snag resolution. Does not write `src/` or `tests/`.
...
- Runs per-file checks (lint, style — **and naming against Architecture's `Ubiquitous language` section**: flag a divergent name or a synonym for an already-named referent) continuously during active work
...
2. **Code speaks the ubiquitous language.** Names in `src/`/`tests/` use the project's terms (Architecture's `Ubiquitous language` section), one name per referent — no synonyms, no per-layer renaming. The Architect and CQ check it. See `~/.claude/wag/references/ubiquitous-language.md`.
3. **File ownership is strict.** Dev writes `src/`, Tester writes `tests/`, Architect writes `.wag/` docs only. CQ writes nothing.
