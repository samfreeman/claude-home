...
# WAG Review — Senior Architect Review
...
The senior architect is deliberately **not** the in-team Architect from `/wag:dev`. The embedded Architect watched the code being written; this reviewer arrives cold. Both perspectives have value — `/wag:review` is the cold one.

## Usage

```
/wag:review              # default: review the active feature branch from state.json
/wag:review <PR#>        # bare integer → resolve to PR's headRefName
/wag:review <branch>     # explicit branch name
```
...
4. **Halt on open snags.** Standard WAG pre-flight check. Scan `.wag/snags/` for `**Status:** open`. If any exist, surface the halt and drive the snag-resolution protocol (`~/.claude/wag/workflows/snag-resolution.md`) inline before continuing. Open snags block all WAG work.
...
Brief the reviewer: if a finding traces upstream (the code follows the ADR but the ADR itself is wrong because Architecture/PRD is wrong), flag a **snag candidate** in the report. The reviewer does NOT capture or resolve snags — `/wag:review` decides with the user.
...
```
<!-- /wag:review — senior architect, independent of the in-team Architect -->
```
...
2. Ask whether to drive the snag-resolution protocol (`~/.claude/wag/workflows/snag-resolution.md`) for any of them.
3. If yes, drive resolution inline per the standard protocol. The candidate becomes a real snag at that point.
4. If no, the candidate is reviewed and dismissed for this session. Tell the user it can be re-raised later by re-running `/wag:review` or capturing the snag directly via the snag template.
...
> - PR: <URL> (or 'no PR')
>
> [If REQUEST_CHANGES] Required actions are in the report. Address them on `feature/PBI-EEE.PPP`, then re-run `/wag:review` for a follow-up.
...
3. **Snag candidates are not snags.** The reviewer flags; the command and user decide whether to capture and resolve.
