...
**Current position:** Steps 1–8 complete. **Step 9: (a) capability gate PASSED, (b) `mcpServers:` PASSED, (c) ask-bridge PASSED (V3 live), `/wag:hadr 000.004` DESIGN run PASSED end-to-end (ADR on `feature/PBI-000.004`, commit `a990b6e`). hadr is DONE and proven.** **V5 (hdev team-spawn) FAILED and is settled:** a confined subagent cannot be a team lead (hard Claude Code rule); the `disallowedTools` fallback won't fix it. **hdev needs an architecture decision (BLOCKED ON USER):**
  - **Opt 1 — Main session as team lead (docs-recommended).** Main session spawns confined `hwag-dev`/`hwag-cq`/`hwag-tester` workers directly + coordinates. Keeps role separation. ⚠️ Lead (main session) has native `Bash`/`gh` → lead-side merge reverts to *discipline*, not capability (workers stay merge-impossible-by-construction).
  - **Opt 2 — Flatten hdev to a single confined worker (hadr-shaped).** Main session spawns ONE confined agent that does implement+test+self-CQ solo via fs+hwag. **Preserves merge-impossible-by-construction for the only actor touching git** (the confined worker; main session only spawns+waits). Loses dev/cq/tester role separation (micro-quality caught at PR review per design philosophy).
  - **Opt 3 — Opt 1 but main-session-lead is convention-barred from native git/gh** (all git/gh through workers' `hwag.run`). Team + role separation; lead-merge protected by discipline only.
  Open polish: mid-run channel switch; V4 (`--ask:session`); exact ask ceiling.
