...
- [ ] Step 9 — End-to-end dry run on a throwaway PBI (also resolves V3 timeout ceiling + V5 team-coordination tools). **Blocked on restart** (live server frozen with the old broken env). After restart: **first verify the bridge with `mcp__hwag__notify` — NOT curl** (curl is exactly what masked the bug). Then: (a) capability-gate test by spawning the confined `hwag-architect` (no Telegram needed); (c) the `ask`→Telegram→reply round-trip + V3 ceiling. **Note:** `hadr` runs the architect *solo* (self-play), so **V5/team-coordination is exercised by `hdev`, not `hadr`.** Target still TBD — proposed: an isolated throwaway project under `~/source` (the real `cs-bounce`/`coin-bounce` backlogs use flat `PBI-NNN`, but `hadr` expects the `EEE.PPP` scheme → would mismatch).

**Current position:** Steps 1–8 complete; env-wiring bug found + fixed (see above). **Next: restart Claude, verify bridge via `mcp__hwag__notify`, then run Step 9.** Open polish: mid-run channel switch; verifications V3/V4/V5.
**Committed:** `dev` @ `b73e21c` (`feat(wag): add headless mode …`), then `1c2f2b8` + `ab9df4c` (env-file fix, this session) — **the two new commits are not yet pushed**. `dist/` is gitignored (repo convention — built locally; exists on this machine). `LEARNING-009.md` left untracked intentionally (unrelated to hwag — handle separately).
