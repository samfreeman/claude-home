...
- [ ] Step 9 — End-to-end dry run on a throwaway PBI (also resolves V3 timeout ceiling + V5 team-coordination tools). Bridge verified (above). **Blocked on restart** to pick up the `name:`-fixed agent defs (the confined subtree cannot spawn until the registry reloads). After restart: (a) capability-gate test by spawning the confined `hwag-architect` (no Telegram needed) — prove it has only `Read/Grep/Glob + mcp__fs__* + mcp__hwag__* + Agent/SendMessage/TaskUpdate`, no native Bash/Write/Edit; (c) the `ask`→Telegram→reply round-trip + V3 ceiling (needs user at phone — coordinate).
  ...
  - **hadr DESIGN RUN PASSED — full pipeline end-to-end (2026-05-29, this session).** `/wag:hadr 000.004` on `~/source/cmn-payonward-portal`. Pre-flight clean (on `dev`, clean tree, no open snags, no active ADR, PBI + docs present; `reasoning/local.md` absent → skipped as designed). Spawned the confined `hwag-architect` (hadr mode, sms channel). It read global reasoning + inputs, **self-played the grill → 7 decisions logged** via `mcp__hwag__log_decision`, **0 escalations** (none hit low-conf+high-blast), wrote the ADR via `mcp__fs__fs_write`, and via `mcp__hwag__run` created `feature/PBI-000.004` + committed. **Verified empirically (main session):** commit `a990b6e` carries 3 files (ADR 138 L, decision log 88 L w/ 7 entries, state.json); `.wag/adr/active/ADR-000.004.md` (status `approved`) + `.wag/decisions/ADR-000.004.md` exist; clean tree; **no PR, no merge, no push**. 33 tool-uses, ~90k tokens, ~3 min. Notable self-made call (#5, decided-self/medium-blast per G-004): mandated `export const dynamic = 'force-dynamic'` so Next can't statically cache a `200 {status:'ok'}` that would be a false-positive liveness signal when the instance is down. **Confirms: confined fs+hwag effector surface is sufficient for a real design run; merge-impossible-by-construction holds in practice.** V5/team-coordination NOT exercised (hadr = solo architect).

  - **V5 SETTLED — FAILED, and the denylist fallback is DEAD (2026-05-29, this session, docs-confirmed).** Ran `/wag:hdev` on approved ADR-000.004 (pre-flight clean; on `feature/PBI-000.004`). The confined `hwag-architect` (a subagent) reported its full schema = `Read, Grep, Glob, mcp__fs__*, mcp__hwag__*, SendMessage` — **`Agent`, `TaskUpdate`, `TeamCreate` all ABSENT** despite `Agent`/`TaskUpdate` being in `tools:`. It correctly halted (no solo work; repo untouched). **claude-code-guide against official docs confirms it's a HARD rule, not a config issue:** (1) sub-agents.md lists `Agent` among tools "**not available to subagents, even when listed in the `tools` field**" — architecturally excluded, not inherited-then-removed; (2) **`disallowedTools` does NOT restore it** (the user's pre-authorized fallback is therefore ruled out — would waste a restart); (3) agent-teams.md: "**No nested teams: teammates cannot spawn their own teams or teammates. Only the lead can manage the team**"; (4) `SendMessage` survives because it's a comms primitive, not a spawn tool. **Consequence: the hdev model as designed (confined subagent lead spawns the confined team) is impossible in Claude Code.** The supported pattern is **the MAIN session is the team lead** (only it has `Agent`/`TeamCreate`); workers are the confined ones. `hadr` is unaffected (solo architect — already PASSED).

  - **V6 SETTLED + hdev WORKS via conductor + plain confined subagents (2026-05-29, this session).** Tried the docs-recommended "main session = Agent Teams lead" path and hit a **second hard wall**: **`TeamCreate` poisons the session into team-mode — every subsequent `Agent` spawn auto-joins the team (even with `team_name` omitted), and teammate-spawned agents do NOT get their def's `mcpServers:` connected.** Proven by actual calls: a teammate `hwag-dev` got `Error: No such tool available: mcp__fs__fs_write` / `mcp__hwag__run`; a second spawn (team_name omitted) auto-joined the team and failed identically. **So `TeamCreate` and MCP-capable workers are mutually exclusive in this harness** — native Agent Teams can give coordination OR MCP effectors, never both. Combined with V5 (confined subagent can't be lead), **native Agent Teams is off the table for confined MCP work.** **The working path (proven end-to-end this session):** conductor = main session (NO `TeamCreate`); workers = **plain confined subagents** (`Agent`, no team context → `mcpServers:` DO connect, exactly as in the hadr run); conductor orchestrates by spawn → read return → resume-by-`agentId` (`SendMessage`); filesystem is the shared state. Ran it: `hwag-dev` (plain) wrote route + test via `mcp__fs__*` and self-gated green via `mcp__hwag__run`; `hwag-cq` (plain, read-only) independently reviewed + re-ran the gate → **APPROVE**; dev committed `3faf083`, pushed, opened/updated **PR #6** (base `dev`) via guarded `gh pr create` + `mcp__hwag__notify`. **No merge tool → merge-impossible-by-construction held.** Role separation (dev writes, cq read-only) preserved. **hdev is functionally PROVEN.** (Also confirmed legible-gate behavior: the confined dev hit its boundary and reported up instead of thrashing.)

**Current position: Step 9 COMPLETE — hwag works end-to-end (hadr + hdev), verified.** `/wag:hadr 000.004` → ADR `a990b6e`; `/wag:hdev` (conductor + plain confined subagents) → impl `3faf083`, CQ-approved, **PR #6 OPEN** (`https://github.com/SamAtPayOnward/cmn-payonward-portal/pull/6`, base `dev`, NOT merged). Merge stays human (run `/wag:review` then merge in UI). **Architecture decision (user, 2026-05-29):** keep hwag working NOW on this conductor+plain-subagent shape; the *proper* substrate — main-session-orchestrator + all-effecting-AND-coordination through MCP-we-write (deny native effectors; build our own intra-agent comms/task-list MCP bus since native Agent Teams is unusable here) + per-agent identity (`{name,project,epic/pbi,branch}`) on every tool call + async-emit to a UI — is a **separate, bigger infra effort = the wag3 substrate** (kwiki: `cooperating-harness-pattern` × `multi-team-agent-architecture` × `event-sourced-agent-ui`). **Dogfood plan:** once hwag works (now), use it to build a new Next.js project that hosts project-local MCP servers in `.claude/` + an observability UI (who-called-what); refine MCPs + hwag commands + UI together; hwag migrates onto it.
  - **Pre-existing repo issue (NOT this PBI):** `cmn-payonward-portal` has a failing `src/modules/donation/schemas/__tests__/branding-config.test.ts` (from PBI-001) + a lint warning in `src/lib/branding.ts` (`_institutionId` unused) — predate this branch, flagged for separate handling.
  - Open polish (hwag): mid-run channel switch; V4 (`--ask:session`); exact ask ceiling; the `/wag:hdev` command still encodes the dead confined-architect-as-lead path → rewrite to conductor + plain-subagents to match what was proven.
