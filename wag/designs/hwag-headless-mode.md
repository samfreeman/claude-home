...
- [ ] Step 9 — End-to-end dry run on a throwaway PBI (also resolves V3 timeout ceiling + V5 team-coordination tools). Bridge verified (above). **Blocked on restart** to pick up the `name:`-fixed agent defs (the confined subtree cannot spawn until the registry reloads). After restart: (a) capability-gate test by spawning the confined `hwag-architect` (no Telegram needed) — prove it has only `Read/Grep/Glob + mcp__fs__* + mcp__hwag__* + Agent/SendMessage/TaskUpdate`, no native Bash/Write/Edit; (c) the `ask`→Telegram→reply round-trip + V3 ceiling (needs user at phone — coordinate). **Note:** `hadr` runs the architect *solo* (self-play), so **V5/team-coordination is exercised by `hdev`, not `hadr`.** Target still TBD — proposed: an isolated throwaway project under `~/source` (the real `cs-bounce`/`coin-bounce` backlogs use flat `PBI-NNN`, but `hadr` expects the `EEE.PPP` scheme → would mismatch).

**Current position:** Steps 1–8 complete; env-wiring bug fixed + **verified live** (`notify` reached phone via real MCP path). Agent-discovery bug (missing `name:`) found + fixed → **needs one more restart** before Step 9 can spawn the confined subtree. **Next: restart Claude, then run Step 9 (a) capability gate, then coordinate (c) the ask round-trip.** Open polish: mid-run channel switch; verifications V3/V4/V5.
**Committed:** `dev` @ `b73e21c` (`feat(wag): add headless mode …`), then `1c2f2b8` + `ab9df4c` (env-file fix). `dist/` is gitignored (repo convention — built locally; exists on this machine). `LEARNING-009.md` left untracked intentionally (unrelated to hwag — handle separately).
