...
**Current position:** Steps 1–8 complete. **Step 9: (a) capability gate PASSED, (b) `mcpServers:` MCP-resolution PASSED, (c) ask-bridge PASSED (V3 live), and the full `/wag:hadr 000.004` DESIGN run PASSED end-to-end (ADR on `feature/PBI-000.004`, commit `a990b6e`).** Both wiring bugs (`name:`, `mcpServers:`) fixed + confirmed live. **Next: `/wag:hdev` on the approved ADR-000.004 — the V5 team-coordination test.** ⚠️ Open V5 risk: the (b) probe showed `Agent` absent from the confined architect's schema ("subagents cannot spawn subagents") — may block the architect standing up the dev/cq/tester team; agent-teams ARE enabled in this harness so it may work anyway — settle empirically. **Fallback if team-spawn fails:** switch confined defs to `disallowedTools: Bash,Write,Edit,NotebookEdit` (denylist preserves coordination tools, still removes native effectors). Security property (no native effectors → no merge) holds either way. Open polish: mid-run channel switch; V4 (`--ask:session`); exact ask ceiling.
