...
wagui server (port 3099)
    └── POST /api/v1/cop
            ...
            ▼
cop.db (SQLite)
    └── rules table (app-agnostic checks)
```

## Acceptance Criteria

- [ ] `wag_cop` MCP tool added to wag-mcp
- [ ] `/api/v1/cop` endpoint in wagui server
- [ ] cop.db created with rules table
- [ ] Default rules seeded:
  ...
  - Git status clean
- [ ] `wag_clear` blocked unless `wag_cop` passed
- [ ] Cop returns specific failure messages
- [ ] Tests written for cop validation logic
...
The cop database is separate from app-specific wagui.db. It accumulates rules across projects.
